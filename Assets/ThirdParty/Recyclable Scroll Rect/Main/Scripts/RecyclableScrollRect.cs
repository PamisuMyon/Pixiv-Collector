...
namespace PolyAndCode.UI
{
    ...
    public class RecyclableScrollRect : ScrollRect
    {
        ...
        */
        ...
        public T[] GetCells<T>(bool inBounds) where T : MonoBehaviour, ICell
        {
            var cells = GetComponentsInChildren<T>();
            if (inBounds)
            {
                var bounds = _recyclingSystem.RecyclableViewBounds;
                List<T> list = new List<T>();
                foreach (var cell in cells)
                {
                    var rect = cell.GetComponent<RectTransform>();
                    if (rect.MinX() >= bounds.min.x - 1
                        && rect.MinY() >= bounds.min.y - 1
                        && rect.MaxX() <= bounds.max.x + 1
                        && rect.MaxY() <= bounds.max.y + 1)
                        list.Add(cell);
                }
                cells = list.ToArray();
            }
